...
void MotorControl::InitialWindowSetup()
{
    ...
    else
    {
        ...
        {
            enableStepper();
            Serial.println("Initially closing window...");
...
void MotorControl::HandleMotorState()
{
    ...
    else
    {
        ...
        if (triggered)
        {
            ...
            if (currentWindowState != WindowState::CLOSED 
                && currentWindowState != WindowState::OPEN 
                && currentWindowState != WindowState::CLOSING_ERROR 
                && currentWindowState != WindowState::OPENING_ERROR)
            {
...
void MotorControl::setCurrentWindowState(WindowState newState)
{
    ...
    Serial.print("New window state is ");
    Serial.println(getWindowStateString(currentWindowState));
}
...
{
    Serial.print("Requested motor state is ");
    Serial.println(getMotorStateString(requestedState));
    if (requestedState == MotorState::OPENING)
    {
        ...
        {
            Serial.println("Window is already open.");
            requestedMotorState = MotorState::STOPPED;
            setCurrentWindowState(WindowState::OPEN);
        }
    }
    else if (requestedState == MotorState::CLOSING)
    {
        ...
        {
            Serial.println("Window is already closed.");
            requestedMotorState = MotorState::STOPPED;
            setCurrentWindowState(WindowState::CLOSED);
        }
    }
    ...
    {
        Serial.println("Stopping window.");
        requestedMotorState = MotorState::STOPPED;
    }
}
